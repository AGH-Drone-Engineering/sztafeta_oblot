...
class GPSDataViewer:
    ...
    def main(self):
        ...
        server_port = st.number_input("Enter server port", value=14551, min_value=1, max_value=65535)  # New field for server port
        ...
        if st.button("Upload Mission"):
            if not st.session_state.gps_data.empty:
                ...
                try:
                    response = requests.post(url, json={"data": data_json, "ip": ip_address, "port": server_port, "height": height_operation})
