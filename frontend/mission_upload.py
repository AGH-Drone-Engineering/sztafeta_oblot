...
class GPSDataViewer:
    ...
    def add_gps_point(self, lat, lon, alt, delay, drop, servo, drop_delay, index=None):
        ...
        if index is not None:
            ...
            st.session_state.gps_data = pd.concat(
                [st.session_state.gps_data, new_point], ignore_index=True
            )

    def remove_gps_point(self, index):
        st.session_state.gps_data = st.session_state.gps_data.drop(index).reset_index(
            drop=True
        )
    ...
    def show_map(self):
        ...
        if not data.empty:
            ...
            folium.PolyLine(
                locations=data[["latitude", "longitude"]].values.tolist(), color="blue"
            ).add_to(map_)
    ...
    def main(self):
        ...
        if action == "Add Waypoint":
            ...
            lat = st.number_input(
                "Latitude", value=53.01907010, format="%.8f", step=0.0000001
            )  # Default lat
            lon = st.number_input(
                "Longitude", value=20.88029020, format="%.8f", step=0.0000001
            )  # Default lon
            alt = st.number_input("Altitude (m)", format="%.2f", step=0.01)

            delay = st.number_input(
                "Delay at this point (seconds)", min_value=0, step=1
            )
            drop = st.checkbox("Payload drop at this point?")
            servo = (
                st.selectbox("Select Servo for drop", options=[1, 2, 3, 4])
                if drop
                else None
            )
            drop_delay = (
                st.number_input("Delay before drop (seconds)", min_value=0, step=1)
                if drop
                else None
            )
            ...
            if st.button("Add Point"):
                ...
                st.success(
                    f"Point added: ({lat}, {lon}, {alt}m) at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
            ...
            if st.button("Add Delay"):
                self.add_gps_point(
                    np.nan, np.nan, np.nan, delay, np.nan, np.nan, np.nan
                )
                st.success(
                    f"Delay added: {delay} seconds at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
        ...
        if not st.session_state.gps_data.empty:
            ...
            point_to_remove = st.selectbox(
                "Select a point to remove", st.session_state.gps_data.index
            )
        ...
        server_port = st.number_input(
            "Enter server port", value=14550, min_value=1, max_value=65535
        )  # New field for server port
        height_operation = st.number_input(
            "Height Operation (m)", value=10.0, step=0.1
        )  # New field for height operation
        if st.button("Upload Mission"):
            if not st.session_state.gps_data.empty:
                url = f"http://0.0.0.0:8001/upload"  # Use user-specified IP address and port
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.error(
                            f"Failed to upload data. Server responded with status code {response.status_code}."
                        )
...
def create_map(map_key, location):
    ...
    m = folium.Map(
        location=location[:2], zoom_start=13
    )  # Use only lat and lon for the map

    # Add a marker for the current point with altitude
    folium.Marker(
        location=location[:2],
        popup=f"({location[0]:.8f}, {location[1]:.8f}, {location[2]}m)",
    ).add_to(m)

    # Display the map and capture the clicked location
    map_data = st_folium(
        m, width=700, height=500, key=f"{map_key}_map"
...
page = st.sidebar.radio(
    "Select a Map", ["GPS Data Viewer", "Map 1", "Map 2", "Map 3", "Map 4"]
)
...
if page == "GPS Data Viewer":
    ...
    with col2:
        ...
        ip_address = st.text_input(
            "External Server IP Address", value="192.168.18.90", key=external_ip_key
        )
        ...
        if (
            lat_input != current_location[0]
            or lon_input != current_location[1]
            or alt_input != current_location[2]
        ):
        ...
        if st.button("Upload Mission"):
            ...
            url = "http://0.0.0.0:8001/streamlit-coordinates"
